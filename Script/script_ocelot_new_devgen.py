...
HMCC: 27-03-17 Fixing f1st bug after the creation of the input file.  Fixing bug of the lattice file generated by OCELOT
HMCC: 17-05-17: Fixing bug of lattice file (planar- helical undulator, aw0 - undulator parameter scaling factor)
HMCC: 09-06-17: Fixing bug of lattice file (planar - helical undulator), when a GENESIS file is present.
'''
#################################################
        ...
        for line in f:
            if not line.strip():
                ...
                if(splitLine[0][:].endswith('file') and splitLine[0][:].startswith('mag')):
                    ...
                    val_atr = int(float(splitLine[-1].replace('=',"")))
...
def read_input_file(f_path):
    ...
    f.close()
    A_content['f1st']=int(A_content['fl'])/2 
    if A_content['in_gen']==0:
        ...
        for attr in A_inp.__dict__.keys():
            if (attr in A_arg_int) or (attr in A_arg_float) or (attr=='nbins') or (attr=='xlamds'): 
...
def undulator_design(A_contents):
   ...
   # Calculation of the Undulator parameter from the Photon and Beam Energies)
    if (A_contents['i_scan']==1 and A_contents['parameter']=='aw0') or (A_contents['in_gen']==1):
        und.Kx = kw0
    else:
...
def main():
    ...
    for in_put in A_input:
        if (in_put in A_simul) or (in_put in A_und) or (in_put in A_td) or (in_put =='xlamds'):
