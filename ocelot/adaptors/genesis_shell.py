...
def run_genesis(inp, launcher, read_level=2, assembly_ver='pyt', debug=1):
    ...
    if launcher.host.startswith('crb'):
        setattr(launcher, 'shell', launcher.dir + getattr(launcher, 'shell'))
        header = ['#!/bin/bash\n',
                  '#BSUB -o ' + inp.outputfile + '\n',
                  '#BSUB -e ' + inp.run_dir + 'run.' + str(inp.runid) + '.err\n',
                  '#BSUB -R "span[ptile=24]"\n',
                  '#BSUB -n 96\n',
                  '#BSUB -J genesis\n',
                  'BSUB -W 180\n']
        with open(launcher.shell, 'a') as f_shell:
            f_shell.writelines(header)
...
def get_genesis_launcher(launcher_program=None):
    ...
    setattr(launcher,'host',host)#HMCC
    ...
    if launcher_program != '':
        ...
        elif get(launcher,'host').startswith('crb'):
            command_genesis = "mpiexec.hydra -np 96 ${MYJOB} < tmp.cmd"
        else:
            command_genesis = launcher_program
        launcher.program = command_genesis
        ###############
        # HMCC comment launcher.program = launcher_program
    else:
