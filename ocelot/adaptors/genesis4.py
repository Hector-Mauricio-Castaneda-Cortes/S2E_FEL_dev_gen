...
from ocelot.adaptors.genesis import GenesisElectronDist,RadiationField,filename_from_path #HMCC
#from ocelot.common.logging import * comment HMCC

###_logger = logging.getLogger('ocelot.gen4') comment HMCC
...
def get_genesis4_launcher(launcher_program='genesis4', launcher_argument=''):
    ...
    launcher_program='/opt/Genesis/bin/genesis4'
    launcher.argument = launcher_argument

    launcher.mpiParameters = '-x PATH -x MPI_PYTHON_SITEARCH -x PYTHONPATH'  # added -n
    launcher.program="mpiexec -np 25" + str(launcher.mpiParameters) +" " + launcher_program + " run.0.inp | tee log "
...
def write_dfl4(dfl, file_path='sample.dfl.h5'):
    """
...
def write_gen4_lat(lat, filePath, line_name='LINE', l=np.inf):
    from ocelot.cpbd.elements import Undulator, Drift, Quadrupole, Chicane, UnknownElement
...
#            print(s)
            ...
            element_name = element_num + 'C'
            s = '{:}: CHICANE = {{l = {:}, lb = {:}, ld = {:.6f}, delay = {:}}};'.format(element_name, element.l, element.lb, element.ld, element.delay)
...
class Genesis4ParticlesDump_old:
    '''
    Genesis particle *.dpa files storage object
    Each particle record in z starts with the energy of all particles
    followed by the output of the particle phases,
...
class Genesis4Output_old:
    ...
    @property
    def rad_power(self):
        return np.array(self.h5['Field/power'])
...
def get_genesis4_launcher_old(launcher_program='genesis4', launcher_argument=''):
    '''
    Returns MpiLauncher() object for given program
    '''
    host = socket.gethostname()
    ...
    # launcher.program = '/data/netapp/xfel/products/genesis/genesis'
    # launcher.argument = ' < tmp.cmd | tee log'
    launcher.program="mpirun -np 25" + str(launcher.mpiParameters) +" " + launcher_program + " run.0.inp | tee log "

    return launcher

def read_gout4_old(filePath):
    ...
    out.z = np.array(out.h5['Lattice/zplot'][:])
    out.zlat = np.array(out.h5['Lattice/z'][:])
...
def dpa42edist_old(dpa, n_part=None, fill_gaps=1, debug=1):
    fill_gaps = 0
    '''
...
def read_dpa42parray_old(filePath, N_part=None, fill_gaps=True):
...
def write_gen4_lat_old(lat, filePath, line_name='LINE', l=np.inf):
...
def write_edist_hdf5_old(edist, filepath):
...
def read_edist_hdf5_old(filepath, charge):
