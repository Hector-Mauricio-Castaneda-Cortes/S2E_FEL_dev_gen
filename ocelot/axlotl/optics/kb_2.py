...
def init_geometry():
    ...
    m1 = EllipticMirror(r=[0,0,0*cm], size=[10*cm,50*cm,30*mm], no=[1.0,0.0,0], a=[5*m, 1.*cm ], id="m1")
    m1.roll = np.pi/2
    a1 = Aperture(r=[0,0,5*m], size=[0.1*m,0.1*m,0.20*m], no=[0.0,0,1], d=[10*mum, 10*mum], id="a1")
    geo = Geometry([m1, a1])
...
r = Ray(r0=[a[1],0.0, -sqrt(a[0]**2 - a[1]**2)], k=[-a[1]/a[0] * (1+ 0.0),0.0, 1])
trace_ray(r, geo)
rays.append(r)
'''
