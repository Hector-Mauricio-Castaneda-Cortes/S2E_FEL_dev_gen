...
from ocelot.gui.genesis_plot import plot_edist,plot_dfl,plot_wigner
...
def plot_gen4_out_all(handle=None, savefig='png', showfig=False, choice=(1, 1, 1, 1, 10, 1, 0, 0, 0, 0, 0, 10, 1),
    ...
    #HMCCif savefig != False:
        #HMCCif debug > 0:
          #HMCC  _logger.info('{}plots recorded to *. {} files'.format(ind_str, savefig))

    if showfig:
        #HMCCif debug > 0:
            #HMCC_logger.info(ind_str + 'showing plots, close all to proceed')
        plt.show()
    # else:
    # plt.close('all')
...
def plot_gen4_out_z(out, z=np.inf, params=['rad_power+el_current', 'el_energy+el_espread+el_bunching', 'rad_spec'],
    ...
    #HMCC_logger.info('plotting bunch profile at z={:.3f} [m]'.format(z))
    ...
    if not out.tdp:
        #HMCC_logger.error('plotting bunch profile is not applicable for steady-state')
        print ('plotting bunch profile is not applicable for steady-state')
        return
