...
class MpiLauncher(Launcher):
    def __init__(self):
        ...
        self.nproc = 0
        self.mpiParameters = ""
        self.shell = 'genesis.bluewonderphase2.intel-impi.lsf'
    ...
    def launch(self):
        ...
        ########## #HMCC ########################      
        #HMCC comment command = 'mkdir -p '+ self.dir + '; ' + 'cd '+ self.dir + '; '+self.program
        #### HMCC Hartree modification
        if self.host.startswith('crb'):
            print('########## Generation of the script ###########')
            os.system('mkdir -p '+ self.dir)
            with open(self.shell, 'a') as f_shell:
                f_shell.writelines(['cd '+ self.dir + '\n',
                       'export MYHOME=`pwd`\n',
                       '\n\n\n',
                       '# setup modules\n',
                       ...
                       'module load genesis\n\n\n',
                       'export MYJOB=genesis_mpi\n\n\n',
                       'export I_MPI_HYDRA_BOOSTRAP=lsf\n',
                       self.program])
            f_shell.close()
        else:
            print (command)
        ...
        #HMCC comment print(command)
        #HMCC comment command = 'mkdir -p ' + self.dir + '; ' + 'cd ' + self.dir + '; ' + self.program
        # HMCC comment os.system(command)
        t2 = time.time()
        print ('execution time ', t2 - t1, ' sec')
